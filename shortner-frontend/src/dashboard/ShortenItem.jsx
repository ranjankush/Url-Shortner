...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
  ...
  const fetchMyShortUrl = async () => {
    setLoader(true);
    try {
      const start = "2024-12-01".split("T")[0]; // optional safety
      // const end = "2025-12-31".split("T")[0];
            ...
            {loader ? (
              ...
              <>
                {analyticsData.length === 0 && (
                  <div className="absolute flex flex-col inset-0 text-center px-4 justify-center sm:items-center items-end  w-full left-0 top-0 bottom-0 right-0 m-auto">
                    <h1 className="sm:w-96 w-[90%] sm:ml-6 text-slate-800 font-serif sm:text-2xl text-[15px] font-bold mb-1">
                      No Data For This Time Period
                    </h1>
                    <h3 className="sm:w-96 w-[90%] sm:ml-6 text-center sm:text-lg text-[12px] text-slate-600 ">
